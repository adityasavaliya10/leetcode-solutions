...
Given an int array `candies`, where each element represents the number of candies a child has, and an integer `extraCandies`, determine for each child whether they can have the greatest number of candies among all children if they are given all the extra candies.
...
5. Append `True` or `False` to the answer list by 'append' function.
